...
int main(void)
{
  ...
  debounceFSM_init();
  KeyStatus_t keystatus = KEY_NO_CHANGES;
  while (1)
  {
	debounceFSM_update();
	keystatus = readKey();
	if (keystatus != KEY_NO_CHANGES){
		uartSendString(getKeyStatusMsg(keystatus));
    }
  }
}
...
const char* getKeyStatusMsg(KeyStatus_t status){
	switch(status){
	case KEY_FALLING_EDGE_DETECTED:return "(↓) Flanco descendente detectado\r\n";
	case KEY_RISING_EDGE_DETECTED:return  "(↑) Flanco ascendente  detectado\r\n";
	default:break;
	}
